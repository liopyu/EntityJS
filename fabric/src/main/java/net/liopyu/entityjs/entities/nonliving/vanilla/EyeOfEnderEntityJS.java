...
public class EyeOfEnderEntityJS extends EyeOfEnder implements IProjectileEntityJS {
    ...
    public void signalTo(BlockPos pos) {
        ...
        if (horizontalDistance > 12.0) {
            this.tx = this.getX() + deltaX / horizontalDistance * 12.0;
            this.tz = this.getZ() + deltaZ / horizontalDistance * 12.0;
        ...
        this.surviveAfterDeath = builder.survivalChance != null
                ? this.random.nextFloat() < builder.survivalChance
    ...
    public void tick() {
        ...
        this.setXRot(Projectile.lerpRotation(this.xRotO, (float) (Mth.atan2(movement.y, horizontalSpeed) * Mth.RAD_TO_DEG)));
        this.setYRot(Projectile.lerpRotation(this.yRotO, (float) (Mth.atan2(movement.x, movement.z) * Mth.RAD_TO_DEG)));
        if (!this.level().isClientSide) {
            double targetDeltaX = this.tx - nextX;
            double targetDeltaZ = this.tz - nextZ;
            float targetDistance = (float) Math.sqrt(targetDeltaX * targetDeltaX + targetDeltaZ * targetDeltaZ);
            float targetAngle = (float) Mth.atan2(targetDeltaZ, targetDeltaX);
            double horizontalVelocity = Mth.lerp(0.0025, horizontalSpeed, targetDistance);
            ...
            movement = new Vec3(
                    Math.cos(targetAngle) * horizontalVelocity,
                    verticalVelocity + (verticalDirection - verticalVelocity) * 0.014999999664723873,
                    Math.sin(targetAngle) * horizontalVelocity
            );
            ...
            if (this.isInWater()) {
                for (int i = 0; i < 4; ++i) {
                    this.level().addParticle(ParticleTypes.BUBBLE,
                            nextX - movement.x * 0.25, nextY - movement.y * 0.25,
                            nextZ - movement.z * 0.25, movement.x, movement.y, movement.z);
                }
            } else {
                this.level().addParticle(ParticleTypes.PORTAL,
                        nextX - movement.x * 0.25 + this.random.nextDouble() * 0.6 - 0.3,
                        nextY - movement.y * 0.25 - 0.5,
                        nextZ - movement.z * 0.25 + this.random.nextDouble() * 0.6 - 0.3,
        ...
        if (!this.level().isClientSide) {
            ...
            ++this.life;
            if (this.life > 80 && !builder.disableDefaultDeathLogic) {
