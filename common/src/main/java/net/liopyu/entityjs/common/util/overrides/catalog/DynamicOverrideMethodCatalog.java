...
// Common-only metadata used by loader-specific dynamic override runtimes.
...
public final class DynamicOverrideMethodCatalog {
    ...
    public static String rejectionReason(Class<? extends Entity> baseClass, String key) {
        return catalog(baseClass).ineligibleMethods().getOrDefault(key, "it is not an overridable instance method on the " + EntityJSPlatform.scriptClassName(baseClass) + " class tree");
    }
    ...
    public static List<String> overrideKeys(Class<? extends Entity> baseClass) {
        ...
        return catalog(baseClass).methods().values().stream()
                .map(MethodSpec::scriptKey)
                .distinct()
                .toList();
    }
        ...
        while (type != null) {
            for (Method method : type.getDeclaredMethods()) {
                ...
                if (isImplementableAbstractMethod(baseClass, method)) {
                    abstractMethods.put(key, methodSpec);
                }
                String reason = ineligibleReason(baseClass, method);
                if (reason == null) {
                    addEligibleMethod(methods, methodSpec);
                } else {
                    addIneligibleMethod(ineligibleMethods, methodSpec, reason);
                }
            }
        ...
        for (Class<?> interfaceType : type.getInterfaces()) {
            ...
            for (Method method : interfaceType.getMethods()) {
                ...
                if (isConcreteInterfaceMethod(method)) {
                    ...
                    removeMethod(methods, ineligibleMethods, MethodSpec.from(method, key));
    ...
    private static void addEligibleMethod(Map<String, MethodSpec> methods, MethodSpec methodSpec) {
        methods.put(methodSpec.key(), methodSpec);
        if (!methodSpec.scriptKey().equals(methodSpec.key())) {
            methods.putIfAbsent(methodSpec.scriptKey(), methodSpec);
        }
    }

    private static void addIneligibleMethod(Map<String, String> ineligibleMethods, MethodSpec methodSpec, String reason) {
        ineligibleMethods.put(methodSpec.key(), reason);
        if (!methodSpec.scriptKey().equals(methodSpec.key())) {
            ineligibleMethods.putIfAbsent(methodSpec.scriptKey(), reason);
        }
    }

    private static void removeMethod(Map<String, MethodSpec> methods, Map<String, String> ineligibleMethods, MethodSpec methodSpec) {
        methods.remove(methodSpec.key());
        ineligibleMethods.remove(methodSpec.key());
        if (!methodSpec.scriptKey().equals(methodSpec.key())) {
            methods.remove(methodSpec.scriptKey());
            ineligibleMethods.remove(methodSpec.scriptKey());
        }
    }

    private static String scriptKey(Method method, String runtimeKey) {
        DynamicOverrideMethodMetadata metadata = methodMetadata(method);
        return metadata == null
                ? runtimeKey
                : scriptKey(metadata, runtimeKey);
    }

    private static String scriptKey(DynamicOverrideMethodMetadata metadata, String fallback) {
        String descriptor = metadata.scriptDescriptor();
        int close = descriptor.indexOf(')');
        if (descriptor.isEmpty() || descriptor.charAt(0) != '(' || close < 0) {
            return fallback;
        }
        StringBuilder builder = new StringBuilder(metadata.scriptName()).append('(');
        int index = 1;
        boolean first = true;
        while (index < close) {
            int arrayDepth = 0;
            ...
            if (typeCode == 'L') {
                ...
                if (semicolon < 0 || semicolon > close) {
                    return fallback;
                }
                typeName = descriptor.substring(index, semicolon).replace('/', '.').replace('$', '.');
                index = semicolon + 1;
            } else {
                typeName = primitiveDescriptorName(typeCode);
                if (typeName == null) {
                    return fallback;
                }
            }
            ...
            builder.append("[]".repeat(arrayDepth));
        }
        return builder.append(')').toString();
    }

    private static String primitiveDescriptorName(char descriptor) {
    ...
    private static String methodLabel(Method method) {
        DynamicOverrideMethodMetadata metadata = methodMetadata(method);
        String owner = metadata == null
                ? method.getDeclaringClass().getSimpleName()
                : simpleScriptOwner(metadata.scriptOwner());
        return owner + "#" + stableMethodName(method);
    }

    private static String simpleScriptOwner(String owner) {
        int separator = Math.max(owner.lastIndexOf('/'), owner.lastIndexOf('.'));
        return owner.substring(separator + 1).replace('$', '.');
    }

    private static boolean isIntentionallyHidden(Method method) {
        String name = stableMethodName(method);
    ...
    private static String stableMethodName(Method method) {
        DynamicOverrideMethodMetadata metadata = methodMetadata(method);
        return metadata == null ? method.getName() : metadata.scriptName();
    }

    private static DynamicOverrideMethodMetadata methodMetadata(Method method) {
        return EntityJSPlatform.dynamicOverrideMethodMetadata(method, descriptor(method));
    }
    ...
    public record MethodSpec(String key, String scriptKey, String name, String descriptor, Class<?> returnType, Class<?>[] parameterTypes, String[] parameterNames, String superBridgeName, int access, boolean abstractMethod) {
        static MethodSpec from(Method method, String key) {
            return new MethodSpec(
                    key,
                    DynamicOverrideMethodCatalog.scriptKey(method, key),
                    method.getName(),
