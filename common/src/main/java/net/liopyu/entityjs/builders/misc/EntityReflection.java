...
public class EntityReflection {
     ...
     * reflection path from being repeated if subclass generation is reintroduced.
     *
    ...
    public static Class<? extends Entity> createEntityClass(Class<? extends Entity> baseClass) {
        if (EntityJSPlatform.isCustomAnimatableClass(baseClass)) {
            return baseClass;
        }
    ...
    public static Class<? extends Entity> rendererEntityClass(Class<?> rendererClass) {
        ...
        while (current != null && current != Object.class) {
            ...
            if (genericSuperclass instanceof ParameterizedType parameterizedType) {
                ...
                if (rawSuperclass.getName().equals(runtimeClassName(ENTITY_RENDERER_CLASS))) {
            ...
            } else if (rawSuperclass.getName().equals(runtimeClassName(ENTITY_RENDERER_CLASS))) {
                return null;
            }
    ...
    public static Class<?> resolveClassName(Object id, String role, String className) {
        return resolveClassName(id, role, className, EntityJSPlatform.isClientEnvironment());
    }
    ...
    public static Class<?> resolveClassName(Object id, String role, String className, boolean logLoadFailure) {
        ...
        try {
            ...
            return Class.forName(runtimeClassName(resolvedClassName), false, classLoader);
    ...
    public static boolean isClientEnvironment() {
        return EntityJSPlatform.isClientEnvironment();
    }
    ...
    private static boolean extendsClassNamed(Class<?> type, String className) {
        String runtimeClassName = runtimeClassName(className);
        Class<?> current = type;
        while (current != null && current != Object.class) {
            if (current.getName().equals(runtimeClassName)) {
                return true;
            }
            current = current.getSuperclass();
        }
    ...
    private static boolean hasRendererContextConstructor(Class<?> type) {
        String runtimeContextClass = runtimeClassName(ENTITY_RENDERER_CONTEXT_CLASS);
        for (Constructor<?> constructor : type.getDeclaredConstructors()) {
            Class<?>[] parameterTypes = constructor.getParameterTypes();
            if (parameterTypes.length == 1
                    && parameterTypes[0].getName().equals(runtimeContextClass)
    ...
    private static String runtimeClassName(String namedClassName) {
        return EntityJSPlatform.runtimeClassName(namedClassName);
    }
    ...
    public static boolean validateRendererTypeCompatibility(Object id, Class<? extends Entity> entityClass, EntityType<?> rendererType) {
        if (EntityJSPlatform.isClientEnvironment()) {
